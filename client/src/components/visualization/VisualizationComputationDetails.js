...
function VisualizationComputationDetails() {
  ...
  const handleCheckJobStatus = async (jobId, index) => {
    ...
    const apiURL = `${process.env.REACT_APP_BACKEND_URL}/container2/get-job-status/:${jobId}?emailId=${emailId}`;
  ...
  const handleDeleteJob = (jobId) => {
    axios
      .delete(
        `${process.env.REACT_APP_BACKEND_URL}/container2/delete-job/${jobId}`,
        tokenHeaders
      )
      .then((response) => {
