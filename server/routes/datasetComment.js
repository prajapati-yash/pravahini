...
const AIAgentComment = require("../models/aiagent_comment");
...
router.put("/comment", (req, res) => {
  ...
  } else if (req.originalUrl == "/model/comment") {
    collection = modelComment;
  } else {
    collection = AIAgentComment;
  }
...
router.put("/comment/reply", async (req, res) => {
  ...
  if (req.originalUrl == "/dataset/comment/reply") {
    collection = datasetComment;
  } else if (req.originalUrl == "/model/comment/reply") {
    collection = modelComment;
  } else {
    collection = AIAgentComment;
  }
  ...
  if (!newReply.parentOfRepliedCommentId) {
    const result = await collection.updateOne(
      ...
      { arrayFilters: [{ "elem.comId": commentId }] },
      (err, result) => {
        if (err) {
          console.error("Error saving user data:", err);
    ...
    const result = await collection.updateOne(
      ...
      { $push: { "comments.$[elem].replies": newReply } },
      { arrayFilters: [{ "elem.comId": newReply.parentOfRepliedCommentId }] },
      (err, result) => {
        if (err) {
          console.error("Error saving user data:", err);
          res.status(500).json({ error: "Failed to save user reply" });
...
router.get("/comments", async (req, res) => {
  ...
  try {
    if (req.originalUrl == "/dataset/comments") {
      collection = datasetComment;
    } else if (req.originalUrl == "/model/comments") {
      collection = modelComment;
    } else {
      collection = AIAgentComment;
    }
...
router.delete("/comment/delete", async (req, res) => {
  try {
    ...
    if (req.originalUrl == "/dataset/comment/delete") {
      collection = datasetComment;
    } else if (req.originalUrl == "/model/comment/delete") {
      collection = modelComment;
    } else {
      collection = AIAgentComment;
    }
...
router.put("/comment/edit", async (req, res) => {
  ...
  try {
    if (req.originalUrl == "/dataset/comment/edit") {
      collection = datasetComment;
    } else if (req.originalUrl == "/model/comment/edit") {
      collection = modelComment;
    } else {
      collection = AIAgentComment;
    }
    if (!data.parentOfEditedCommentId) {
      ...
      result = await collection.updateOne(
        {
          "comments.comId": data.parentOfEditedCommentId,
          "comments.replies.comId": data.comId,
        },
